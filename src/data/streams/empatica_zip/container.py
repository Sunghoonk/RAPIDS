...
def pull_data(data_configuration, device, sensor, container, columns_to_download):
    print("data_configuration", data_configuration)
    print("device", device)
    print("sensor", sensor)
    ...
    path = (Path(data_configuration["FOLDER"]) / Path(device)).rglob("*.zip")

    print("available_zip: ", available_zipfiles)
